...
export class Renderer {
  ...
  private static fitCamera(): { scale: number; originX: number; originY: number } {
    const corners = [
      Iso.toScreen(0, 0),
  ...
  private projectXY(x: number, y: number): Point {
  ...
  private lift(screen: Point, height: number): Point {
  ...
  private drawJungleBlob(side: TeamSide): void {
    const ctx = this.ctx;
  ...
  private drawLane(points: Point[], color: string, width: number): void {
    if (points.length === 0) {
      return;
    }
    const ctx = this.ctx;
  ...
  private drawCampMarker(camp: { x: number; y: number; name: string }, color: string): void {
    const ctx = this.ctx;
  ...
  private drawObjectiveMarker(point: Point, label: string, color: string): void {
    const ctx = this.ctx;
    const screen = this.project(point);
    ctx.save();
    ctx.strokeStyle = color;
  ...
  private drawEntities(match: Match): void {
  ...
  private drawShadow(pos: Point, radius: number, alpha = 0.32): void {
    const ctx = this.ctx;
  ...
  private drawGroundRing(pos: Point, radius: number, tone: "blue" | "red" | "neutralBlue" | "neutralPurple", filled: boolean): void {
    const ctx = this.ctx;
    const screen = this.project(pos);
    const palette = {
  ...
  private drawToken(pos: Point, radius: number, height: number, fill: string, stroke: string, squash = 0.92): { x: number; y: number } {
    const ctx = this.ctx;
  ...
  private drawTower(tower: Structure): void {
    const ctx = this.ctx;
  ...
  private drawNexus(nexus: Structure): void {
    const ctx = this.ctx;
  ...
  private drawMinion(minion: Minion): void {
  ...
  private drawChampion(champion: Champion): void {
  ...
  private drawLevelBadge(champion: Champion, body: Point): void {
    const ctx = this.ctx;
  ...
  private drawBuffRing(champion: Champion, body: Point): void {
    const ctx = this.ctx;
    ctx.save();
  ...
  private drawChampionLabel(champion: Champion): void {
    const ctx = this.ctx;
  ...
  private drawRecall(champion: Champion): void {
    const ctx = this.ctx;
    const screen = this.project(champion.pos);
  ...
  private drawRespawnMarkers(match: Match): void {
    const deadBySide: Record<TeamSide, Champion[]> = {
  ...
  private drawRespawnMarker(champion: Champion, index: number, count: number): void {
    const base = this.project(BASE_POSITIONS[champion.side as TeamSide]);
  ...
  private drawVFX(match: Match): void {
    const ctx = this.ctx;
  ...
  private drawBar(x: number, y: number, width: number, height: number, percent: number, color: string): void {
    const ctx = this.ctx;
