...
import type { AccountState, ChampionSelection, ChampionTemplate, MatchResult, OwnedChampion, RewardResult, Role, Tier } from "../data/models.js";
import { SaveManager } from "./SaveManager.js";

export class AccountStore {
  account: AccountState;
  private saveManager: SaveManager<AccountState>;
  private rng: () => number;

  constructor(rng: () => number = Math.random, saveManager = new SaveManager<AccountState>(STORAGE_KEY)) {
    this.rng = rng;
  ...
  getSelectedRoster(): ChampionSelection[] {
    ...
    return this.account.selectedTeam
      ...
      .filter((selection): selection is ChampionSelection => Boolean(selection));
  }

  getSelectedByRole(): Partial<Record<Role, ChampionSelection>> {
    const selected: Partial<Record<Role, ChampionSelection>> = {};
  ...
  selectChampion(uid: string): void {
  ...
  toggleFavoriteChampion(uid: string): void {
    this.toggleUidList("favoriteChampionUids", uid);
  }

  toggleLockedChampion(uid: string): void {
    const selection = this.getOwnedWithTemplates().find((candidate) => candidate.owned.uid === uid);
    if (!selection) {
      return;
    }
  ...
  isFavoriteChampion(uid: string): boolean {
    return this.account.favoriteChampionUids.includes(uid);
  }

  isLockedChampion(uid: string): boolean {
  ...
  setSelectedTeam(uids: string[]): void {
  ...
  completeMatch(result: MatchResult): RewardResult {
  ...
  private addAccountXp(amount: number): void {
  ...
  private toggleUidList(key: "favoriteChampionUids" | "lockedChampionUids", uid: string): void {
...
export function createDefaultAccount(): AccountState {
...
export function getTierForLevel(accountLevel: number): Tier {
  let tier: Tier = "D";
...
export function rollChampion(accountLevel: number, account: AccountState, rng: () => number = Math.random): ChampionTemplate | null {
...
function addChampionXp(champion: OwnedChampion, amount: number): void {
...
function getChampionLevelThreshold(level: number): number {
  return 80 + level * 35;
}
