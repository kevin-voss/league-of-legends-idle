...
import { BASE_POSITIONS, JUNGLE_CAMPS, JUNGLE_PATHS, MAP_RAILS, MIN_TOWER_SPACING, TOWER_ATTACK_RANGE, getEnemySide, getLanePath, mirrorPoint } from "../dist/data/MapData.js";
import { CANVAS_WIDTH, MATCH_MAX_SECONDS } from "../dist/data/Constants.js";
...
test("a full match completes within the configured window", () => {
  ...
  assert.ok(result.durationSeconds <= MATCH_MAX_SECONDS + 1);
...
test("opposing minion waves reach T1 towers on the same simulation tick", () => {
  for (const lane of ["top", "mid", "bot"]) {
    const blueTicks = ticksUntilT1Range("blue", lane);
    const redTicks = ticksUntilT1Range("red", lane);
    assert.equal(blueTicks, redTicks, `${lane} T1 arrival tick mismatch`);
  }
});
...
test("jungle camps stay off the lane paths", () => {
  ...
  for (const blueCamp of JUNGLE_CAMPS.filter((camp) => camp.side === "blue")) {
    const redCamp = JUNGLE_CAMPS.find((camp) => camp.id === blueCamp.id.replace("blue", "red"));
    assert.ok(redCamp);
    assert.deepEqual({ x: redCamp.x, y: redCamp.y }, mirrorPoint(blueCamp));
  }
...
test("bot lane uses an L-shaped path through the bottom-right river bend", () => {
  ...
  assert.ok(botBlue[1].x > CANVAS_WIDTH / 2);
  assert.ok(botBlue[2].x > CANVAS_WIDTH / 2);
  assert.ok(botBlue[2].y < BASE_POSITIONS.blue.y);
});

test("red jungle routing is generated from the blue route", () => {
  assert.deepEqual(JUNGLE_PATHS.red, JUNGLE_PATHS.blue.map(mirrorPoint));
});
...
test("destroying both lane turrets makes attackers push and defenders collapse", () => {
  ...
  assert.ok(match.champions.every((champion) => champion.rotationLane === "top"));
  assert.ok(match.champions.filter((champion) => champion.side === "blue").every((champion) => champion.rotationIntent === "push"));
  assert.ok(match.champions.filter((champion) => champion.side === "red").every((champion) => champion.rotationIntent === "defend"));
});
